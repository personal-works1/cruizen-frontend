...
const CATEGORIES = [
  ...
  "Beauty & Skincare", "Food & Drinks", "Books"
]
    ...
    <div className="modalOverlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modalBox">
        ...
        <div className="modalBtns">
          ...
          <button className="submitBtn" onClick={handleTopUp} disabled={loading}>
      ...
      <div className="modalBox">
        ...
        {success ? (
          <div style={{ textAlign: "center", padding: "1rem" }}>
            ...
            <button className="submitBtn" style={{ marginTop: "1rem" }} onClick={onClose}>Done</button>
          </div>
        ) : (
          <>
            <div className="field">
              ...
              <input type="number" name="amount" placeholder="Enter amount"
                value={form.amount} onChange={handleChange} min="100" />
            ...
            <div className="field">
              ...
              <input type="text" name="account_number" placeholder="10-digit account number"
                value={form.account_number} onChange={handleChange} maxLength={10} />
            </div>
            <div className="field">
              <label>Account Name</label>
              <input type="text" name="account_name" placeholder="Account holder name"
                value={form.account_name} onChange={handleChange} />
            </div>
            {error && <p style={{ color: "red", fontSize: "13px" }}>{error}</p>}
            <div className="modalBtns">
              <button className="cancelBtn" onClick={onClose}>Cancel</button>
              <button className="submitBtn" onClick={handleWithdraw} disabled={loading}>
...
function TransferModal({ onClose, balance, onSuccess }) {
  ...
  const [form, setForm]   = useState({ recipient_username: "", amount: "", note: "" })
  const [loading, setLoading] = useState(false)
  const [error,   setError]   = useState("")
  const [success, setSuccess] = useState("")
...
function UploadProductModal({ onClose, onUploaded }) {
  ...
  const [form, setForm] = useState({
    ...
    fake_price: "", category: "", units_left: ""
  })
      ...
      <div className="modalBox" style={{ maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.8rem" }}>
          <div className="field">
            ...
            <input name="price" type="number" value={form.price}
              onChange={handleChange} placeholder="5000" min="1" />
          </div>
          <div className="field">
            <label>Slashed Price <span className="optional">(optional)</span></label>
            <input name="fake_price" type="number" value={form.fake_price}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.8rem" }}>
          ...
          <div className="field">
            ...
            <input name="units_left" type="number" value={form.units_left}
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (reference) {
      axios.get(`${API_URL}/wallet/topup/verify?reference=${reference}`, {
        headers: authHeader
      }).then((res) => {
